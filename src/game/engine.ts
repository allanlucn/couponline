...
import { CHARACTERS, ACTION_META, type ActionType, type Character, type PendingAction } from "./types";
...
export function reduce(state: FullState, action: Action): { state: FullState; events: LogEvent[] } {
  ...
  if (action.kind === "challenge") {
    ...
    if (claimantP.hand.includes(claimingChar)) {
      ...
      s = setPending(s, { ...p0, phase: "lose_influence", loseInfluence: { playerId: action.challengerId, reason: "desafio_perdido" } });
      return { state: s, events };
    } else {
      events.push({ type: "bluff_caught", payload: { claimant, character: claimingChar } });
      s = setPending(s, { ...p0, phase: "lose_influence", loseInfluence: { playerId: claimant, reason: "blefe_pego" } });
      return { state: s, events };
    }
  }
  ...
  if (action.kind === "block") {
    ...
    s = setPending(s, { ...p0, phase: "challenge_block", passed: [], block: { blockerId: action.blockerId, character: action.character } });
    return { state: s, events };
  }
  ...
  if (action.kind === "exchange_return") {
    if (p0.phase !== "exchange_pick" || !p0.exchangeCards) throw new Error("Sem troca em andamento");
...
function finishResolving(s: FullState, events: LogEvent[]): { state: FullState; events: LogEvent[] } {
  ...
  switch (p0.type) {
    ...
    case "coup":
    case "assassinate":
      s = { ...s, pending: { ...p0, phase: "lose_influence", loseInfluence: { playerId: p0.targetId!, reason: "acao" } } };
      return { state: s, events };
    case "exchange": {
      const deck = [...s.deck];
