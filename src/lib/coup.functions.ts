...
export const createRoom = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: { name: string }) => z.object({ name: z.string().min(1).max(24) }).parse(d))
...
export const joinRoom = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { code: string; name: string }) =>
    z.object({ code: z.string().min(4).max(8), name: z.string().min(1).max(24) }).parse(d),
  )
  .handler(async ({ data, context }) => {
    const supa = await admin();
...
async function persistState(roomId: string, prev: FullState, next: FullState, events: { type: string; payload: any }[]) {
  const supa = await admin();
  // rooms
  await supa
    .from("rooms")
    .update({
      ...
      state: { deck: next.deck, pending: next.pending, rngSeed: next.rngSeed },
  ...
  for (const p of next.players) {
    ...
    if (!before || before.hand.join(",") !== p.hand.join(",")) {
      ...
      await supa
        .from("hands")
        .upsert({ player_id: p.id, anon_user_id: (await supa.from("players").select("anon_user_id").eq("id", p.id).single()).data!.anon_user_id, cards: p.hand });
    }
  }
  // events
  ...
  if (events.length) {
    await supa.from("events").insert(events.map((e) => ({ room_id: roomId, seq: seq++, type: e.type, payload: e.payload })));
  }
}
...
export const startGameFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const state = startGame(players.map((p: any) => ({ id: p.id, name: p.name, seat: p.seat })), seed);
    // persist hands
    ...
    await supa
      ...
      .update({
        ...
        state: { deck: state.deck, rngSeed: state.rngSeed, pending: null },
      })
      .eq("id", data.roomId);
    await supa.from("events").insert({ room_id: data.roomId, seq: 1, type: "game_started", payload: {} });
    return { ok: true };
  });
